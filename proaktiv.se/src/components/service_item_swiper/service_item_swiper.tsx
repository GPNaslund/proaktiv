...
const ServiceItemSwiper: React.FC<ServiceSwiperProps> = ({ items }) => {
    ...
    useEffect(() => {
        ...
        window.addEventListener('swiperLinkClick', handleLinkClick as EventListener);
        
        return () => {
            window.removeEventListener('swiperLinkClick', handleLinkClick as EventListener);
        }
    }, [items]);
    ...
    return (
        ...
        >
            <div className={`swiper-navigation ${currentIndex > 0 ? 'show-prev' : ''} ${currentIndex < items.length - 1 ? 'show-next' : ''}`}>
